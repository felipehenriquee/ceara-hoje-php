<?php 
    ...
    if(!isset($URL[1])){
        ...
        $resultTodasNoticias = mysqli_query($connect, "SELECT id, titulo, foto, cidade, tituloConvertido, createdAt, autor FROM noticias  ORDER BY createdAt ASC LIMIT $inicio, $itensPorPagina");
        $resultTotalNoticias = mysqli_query($connect, "SELECT id FROM noticias");
    }
    else{
        $IdCidadeFiltro = pegaIdCidade($connect, $URL[1]);
        $resultTodasNoticias = mysqli_query($connect, "SELECT id, titulo, foto, cidade, tituloConvertido, createdAt, autor FROM noticias WHERE cidade =$IdCidadeFiltro ORDER BY createdAt ASC LIMIT $inicio, $itensPorPagina");
        $resultTotalNoticias = mysqli_query($connect, "SELECT id FROM noticias WHERE cidade =$IdCidadeFiltro");
